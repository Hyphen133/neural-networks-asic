...
`endif
    ...
    // is what has always shipped. 2 = the maximum plus a leaky average of the
    // same log feature, favg += (feat - favg) >> AVG_SHIFT, carried across
    // frame boundaries. The template then reads NBAND*NSTAT features per
    // frame. The average is what the maximum throws away, and on the eight
    // detectors in docs/task_optimization.md it is worth +2.4 to +9.0 test
    // AUC -- more than every cascade change measured, combined.
    parameter NSTAT        = 1,
    parameter AVG_SHIFT    = 3,     // leaky-average time constant, in ticks
                                    // of the band's own decimated clock
...
`endif
  ...
  // Per-band leaky average, rotated in lockstep with fmax so the band under
  // update is always at the head. Unlike fmax it is NOT cleared at the frame
  // boundary: it is an integrator with a time constant of its own, and
  // clearing it every frame would turn it back into a per-frame statistic
  // whose value depends on where the boundary fell.
  ...
  // Leaky average of the same log feature, for the band at the head of the
  // ring: favg += ((feat << AVG_SHIFT) - favg) >>> AVG_SHIFT. The difference
  // is signed and the accumulator is not, so it is widened by one bit and
  // sign-extended explicitly -- yosys zero-extends N'(signed_expr) where the
  // LRM sign-extends, which is the bug that produced a rectified microphone
  // in the cascade above.
  wire signed [AVG_W:0] avg_cur = {1'b0, favg[0]};
  wire signed [AVG_W:0] avg_tgt = {1'b0, {feat, {AVG_SHIFT{1'b0}}}};
  wire signed [AVG_W:0] avg_dif = avg_tgt - avg_cur;
  wire signed [AVG_W:0] avg_sum = avg_cur + (avg_dif >>> AVG_SHIFT);
  wire        [AVG_W-1:0] avg_nx = avg_sum[AVG_W-1:0];
        ...
        S_CASC: begin
          ...
          if (is_tap) begin
            ...
            if (NSTAT > 1) begin
              ...
              favg[NAVG-1] <= casc_due ? avg_nx : favg[0];
            end
          end
          if (casc_last) begin
            if (frame_end) begin
